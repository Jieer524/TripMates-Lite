...
def parse_yes_no(value: object, field_name: str) -> bool:
    ...
    raise ValueError(
        f"{field_name} must use YES or NO. "
        f"Received: {value!r}"
    )
...
def parse_off_days(value: object) -> frozenset[str]:
    ...
    if invalid_days:
        raise ValueError(
            f"Invalid off-day values: "
            f"{sorted(invalid_days)}"
        )
...
class PlanningData:
    ...
    def get_travel_time(
        ...
        try:
            ...
            raise KeyError(
                "Walking-matrix value is missing for "
                f"{from_poi_id} -> {to_poi_id}."
            ) from error
        ...
        if from_poi_id == to_poi_id:
            if not math.isclose(
                ...
                raise ValueError(
                    "Walking-matrix diagonal must be zero."
                )
...
class TravelAdjustments:
    ...
    def apply(
        ...
        if self.global_multiplier <= 0:
            raise ValueError(
                "Global travel multiplier must be positive."
            )
        ...
        if link_multiplier <= 0:
            raise ValueError(
                "Link travel multiplier must be positive."
            )
...
def load_planning_data(
    ...
    if not poi_path.exists():
        raise FileNotFoundError(
            f"POI file not found: "
            f"{poi_path.resolve()}"
        )

    if not matrix_path.exists():
        raise FileNotFoundError(
            f"Walking matrix not found: "
            f"{matrix_path.resolve()}"
        )
    ...
    expected_ids = list(
        range(1, EXPECTED_POI_COUNT + 1)
    )

    if poi_df["poi_id"].tolist() != expected_ids:
        raise ValueError(
            "POI IDs must be ordered exactly from 1 to 50."
        )
    ...
    for row in poi_df.itertuples(index=False):
        poi = POI(
            ...
            opening_minute=parse_clock(
                row.opening_time_24h
            ),

            closing_minute=parse_clock(
            ...
            visit_duration_min=float(
                row.visit_duration_min
            ),

            admission_cost_rm=float(
                row.admission_cost_rm
            ),
    ...
    if list(matrix_df.columns) != expected_matrix_columns:
        raise ValueError(
            "Walking-matrix columns do not align "
            "with POI IDs 1–50."
        )
    ...
    if matrix_df["poi_id"].tolist() != expected_ids:
        raise ValueError(
            "Walking-matrix row IDs do not align "
            "with POI IDs 1–50."
        )

    matrix_df = matrix_df.set_index("poi_id")

    matrix_values = matrix_df.to_numpy(
        dtype=float
    )
    ...
    if matrix_values.shape != (
        ...
        raise ValueError(
            "Walking matrix must have shape 50 × 50."
        )
    ...
    if not math.isclose(
        ...
        raise ValueError(
            "Walking matrix is not symmetric."
        )
...
def decode_route(
    ...
    *,
    ...
    sequence = tuple(
        int(poi_id)
        for poi_id in visit_sequence
    )

    unavailable_ids = {
        int(poi_id)
        for poi_id in unavailable_poi_ids
    }
    ...
    if required_mandatory_ids is None:
        ...
        required_ids = {
            int(poi_id)
            for poi_id in required_mandatory_ids
        }
    ...
    def build_result(
        ...
        return ScheduleResult(
            ...
            finish_clock=format_clock(
                current_time
            ),

            total_cost_rm=current_cost,

            required_mandatory_ids=tuple(
                sorted(required_ids)
            ),

            unavailable_mandatory_ids=tuple(
                sorted(unavailable_mandatory)
            ),

            missing_mandatory_ids=tuple(
                sorted(missing_ids)
            ),
        )
    ...
    if start_poi_id not in data.pois:
        raise KeyError(
            f"Unknown start POI: {start_poi_id}"
        )

    unknown_ids = [
        poi_id
        for poi_id in sequence
        if poi_id not in data.pois
    ]
    ...
    if (
        unavailable_mandatory
        and not allow_unavailable_mandatory
    ):
        return build_result(
            False,
            "Mandatory POIs are unavailable: "
            f"{sorted(unavailable_mandatory)}",
        )
    ...
    if start_poi_is_visit:
        if not sequence:
            return build_result(
                False,
                "The initial route must contain "
                "the starting POI.",
            )
    ...
    elif start_poi_id in sequence:
        return build_result(
            False,
            "The current origin must not be repeated "
            "during bounded replanning.",
        )

    for sequence_index, poi_id in enumerate(
        sequence
    ):
        ...
        opening_window = poi.get_opening_window(
            day_code
        )
        ...
        if opening_window is None:
            return build_result(
                False,
                f"POI {poi_id} ({poi.name}) "
                f"is closed on {day_code}.",
            )

        if (
            sequence_index == 0
            and start_poi_is_visit
        ):
        ...
        if visit_start_min >= closing_min:
            return build_result(
                False,
                f"POI {poi_id} ({poi.name}) "
                "cannot be started before closing.",
            )
        ...
        if visit_end_min > trip_end_min + 1e-9:
            return build_result(
                False,
                f"POI {poi_id} ({poi.name}) "
                "would exceed the 17:00 trip limit.",
            )
        ...
        entry = ScheduleEntry(
            ...
            visit_start_clock=format_clock(
                visit_start_min
            ),

            visit_end_min=visit_end_min,
            visit_end_clock=format_clock(
                visit_end_min
            ),
...
def print_schedule(
    ...
    print(
        f"Requested route: "
        f"{list(result.requested_sequence)}"
    )

    print(
        f"Completed route: "
        f"{list(result.completed_sequence)}"
    )

    print(
        f"Finish time: "
        f"{result.finish_clock}"
    )

    print(
        f"Travel time: "
        f"{result.total_travel_min:.2f} min"
    )

    print(
        f"Waiting time: "
        f"{result.total_waiting_min:.2f} min"
    )

    print(
        f"Visit time: "
        f"{result.total_visit_min:.2f} min"
    )

    print(
        f"Total time used: "
        f"{result.time_used_min:.2f} min"
    )

    print(
        f"Total cost: "
        f"RM{result.total_cost_rm:.2f}"
    )

    print(
        "Missing mandatory POIs: "
        f"{list(result.missing_mandatory_ids)}"
    )
...
def main() -> None:
    ...
    if not result.feasible:
        raise SystemExit(
            "Shared planning-core test failed."
        )
